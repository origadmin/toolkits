...
const _testBootstrapFile = "bootstrap.toml"
...
func TestEnvWithPrefix(t *testing.T) {
	...
	err := codec.EncodeToFile(_testBootstrapFile, envs)
	if err != nil {
		t.Fatal(err)
	}
	...
	c := config.New(config.WithSource(
		file.NewSource(path),
		NewSource([]string{_testBootstrapFile}, prefix1, prefix2),
	))
...
func TestEnvWithoutPrefix(t *testing.T) {
	...
	c := config.New(config.WithSource(
		NewSource([]string{_testBootstrapFile}),
		file.NewSource(path),
	))
...
func Test_env_watch(t *testing.T) {
	...
	source := NewSource([]string{_testBootstrapFile}, prefixes...)
	w, err := source.Watch()
	if err != nil {
