	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p, err := validator.ValidateParams(&types.Config{
				SaltLength: 16,
				Params:     must.Do(codec.DecodeParams(tt.params)),
			}, &Params{})
			...
			if tt.params != "" {
				...
				p2, err := validator.ValidateParams(&types.Config{
					Params:     must.Do(codec.DecodeParams(str)),
					SaltLength: 16,
				}, p.Params)
				if err != nil {
					t.Fatalf("Failed to parse string generated by String() for Params '%s': %p", str, err)
				}
